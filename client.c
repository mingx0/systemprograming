 ...
 *
 * 핵심 구조
 * 1) 클라이언트는 서버에 접속하여 메뉴와 결과를 화면에 출력합니다.
 * 2) 사용자가 입력한 값은 서버로 전송합니다.
 * 3) DB 파일은 절대 직접 열지 않습니다. fopen/fread/fwrite가 없습니다.
 * 4) AF_INET + SOCK_STREAM을 사용하므로 pipe가 아니라 인터넷 소켓(TCP/IP) 방식입니다.
 */
