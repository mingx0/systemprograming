 ...
 *
 * 핵심 구조
 * 1) 서버만 DB 파일(stdinfo.db 등)에 접근합니다.
 * 2) 클라이언트는 DB 파일을 직접 읽거나 쓰지 않고, 서버와 소켓으로만 통신합니다.
 * 3) AF_INET + SOCK_STREAM을 사용하므로 pipe가 아니라 인터넷 소켓(TCP/IP) 방식입니다.
 * 4) accept() 후 fork()를 사용하여 여러 클라이언트 접속을 처리합니다.
 * 5) 메뉴 기능 실행 시에도 fork()를 사용하여 부모/자식 프로세스 구조를 유지합니다.
 * 6) DB 파일을 읽거나 쓸 때 fcntl()로 파일 잠금(lock)과 해제(unlock)를 수행합니다.
 */
